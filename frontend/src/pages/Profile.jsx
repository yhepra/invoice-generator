...
export default function Profile({ user, onUpdateUser, settings }) {
  ...
  const handleChangePassword = async (e) => {
    ...
    if (newPassword !== newPasswordConfirmation) {
        setPasswordMessage({ type: 'error', text: t('newPasswordsDoNotMatch') });
  ...
  return (
    <div className="mx-auto max-w-3xl px-4 py-8 sm:px-6 lg:px-8">
      <div className="space-y-6">
        <div>
            <h1 className="text-2xl font-bold tracking-tight text-gray-900">{t('profileSettings')}</h1>
            <p className="mt-1 text-sm text-gray-500">{t('profileDescription')}</p>
        </div>
        ...
        <div className="bg-white shadow sm:rounded-lg">
          <div className="px-4 py-5 sm:p-6">
            <h3 className="text-lg font-medium leading-6 text-gray-900">{t('changePassword')}</h3>
            <div className="mt-2 max-w-xl text-sm text-gray-500">
              <p>{t('passwordDescription')}</p>
            </div>
            <form className="mt-5 space-y-4" onSubmit={handleChangePassword}>
              <div>
                <label htmlFor="current_password" className="block text-sm font-medium text-gray-700">{t('currentPassword')}</label>
                <div className="mt-1">
                    <input
                        type="password"
              ...
              <div>
                <label htmlFor="new_password" className="block text-sm font-medium text-gray-700">{t('newPassword')}</label>
                <div className="mt-1">
                    <input
                        type="password"
                        name="new_password"
              ...
              <div>
                <label htmlFor="new_password_confirmation" className="block text-sm font-medium text-gray-700">{t('confirmNewPassword')}</label>
                <div className="mt-1">
                    <input
                        type="password"
