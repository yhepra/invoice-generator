...
export default function Template1({ invoice, user }) {
    ...
    <div className="flex w-full flex-col bg-white text-xs text-gray-900 relative font-sans h-full min-h-[297mm]">
      ...
      <div className="relative w-full min-h-[128px] flex items-start justify-between">
        ...
        <div className="absolute top-0 left-0 w-full h-full z-0">
          {/* Orange Top Bar */}
          <div className="absolute top-0 left-0 w-full h-8 bg-orange-500"></div>
          {/* Black polygon using border triangle for html2canvas compatibility */}
          <div className="absolute top-8 left-0 h-24 bg-[#111] md:w-[55%] w-[55%]"></div>
          <div className="absolute top-8 left-[55%] w-0 h-0 border-t-[96px] border-t-[#111] border-r-[40px] border-r-transparent md:border-r-[60px]"></div>
        </div>
        ...
        <div className="relative z-10 flex w-full px-8 pt-10">
          ...
          <div className="w-[45%] text-right pt-2 pr-4 flex flex-col items-end">
            <h1 className="text-4xl font-bold text-orange-500 tracking-wider uppercase mb-3 drop-shadow-sm">
      ...
      <div className="px-8 flex-1 mt-6">
        ...
        <section className="flex justify-between w-full mb-8">
          <div className="w-1/2">
            <h3 className="text-orange-500 font-bold uppercase tracking-wider text-[11px] mb-2">{t("billTo")}</h3>
          ...
          {/* Seller / Payment Info if needed. Emulating visual's "Payment Method" with Seller info to maintain layout */}
          <div className="w-[35%] pl-4 border-l border-gray-200">
            <h3 className="text-orange-500 font-bold uppercase tracking-wider text-[11px] mb-2">{t("from")}</h3>
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="uppercase text-[11px] tracking-wider font-semibold text-white">
                <th className="bg-[#111] py-3 pl-4 pr-2 w-12 text-center rounded-tl-sm">NO.</th>
                <th className="bg-[#111] py-3 px-2">{t("item")}</th>
                <th className="bg-orange-500 py-3 px-2 w-28 text-right">{t("price")}</th>
                <th className="bg-orange-500 py-3 px-2 w-16 text-right">{t("quantity")}</th>
                <th className="bg-orange-500 py-3 px-4 w-32 text-right rounded-tr-sm">{t("total")}</th>
              </tr>
            </thead>
            <tbody>
              {items.map((item, idx) => (
        ...
        <div className="flex justify-between items-start mt-8">
          ...
          <div className="w-[55%] pr-4 space-y-5">
            {details.terms && (
              <div>
                <h4 className="text-orange-500 font-bold uppercase tracking-wider text-[10px] mb-2">{t("terms")}</h4>
                <div
            ...
            {details.notes && (
              <div>
                <h4 className="text-orange-500 font-bold uppercase tracking-wider text-[10px] mb-2">{t("notes")}</h4>
                <div
                  className="wysiwyg-content text-[10px] text-gray-600 leading-relaxed"
          ...
          <div className="w-[40%]">
            ...
            <div className="flex justify-between py-3 mt-1 bg-orange-500 px-4 rounded-sm text-white">
      ...
      <div className="w-full mt-auto mb-4 pb-6 px-8 flex justify-between items-center text-[9px] text-gray-500">
         ...
         <div className="border-t-[3px] border-orange-500 pt-3 w-full flex justify-between">
